...
$$
m_t^{(\ell)} = \beta^{(\ell)} g_t^{(\ell)}
  + \left(1 - \beta^{(\ell)}\right)
    \sum_{i=1}^{L-1} \alpha_i^{(\ell)} g_{t-i}^{(\ell)}
$$
...
$$
s_i^{(\ell)} =
\frac{q_t^{(\ell)} {k_{t-i}^{(\ell)}}^\top}{\sqrt{d_\ell}},
\qquad i \in \{1, \dots, L-1\}
$$

$$
q_t^{(\ell)} = g_t^{(\ell)} W_Q^{(\ell)},
\qquad
...
$$
s_i^{(\ell)} = \cos\!\left(g_t^{(\ell)}, g_{t-i}^{(\ell)}\right),
\qquad i \in \{1, \dots, L-1\}
$$
